...
# Este script irá ler a planilha de uma das atividades - Tsubame, Vôlei ou Zenshin
...
def verificaGrupos(planilha, aba, status, nomeGrupo, imgGrupo):
    ...
    def checkar_ativos(nomeGrupo, lista_ativos):
        # ler grupo do Whatsapp
        lista_junia = read_file(nomeGrupo)
        ...
        title = f"Junias ATIVOS no CADASTRO que não estão no grupo {nomeGrupo[:-5].upper()}: \n\n"

        return title, msg

    # Define a função para checkar quem são os membros do grupo do Whatsapp 
    # A Função recebe um grupo de what'sapp e uma lista de nomes+numeros
    def checkar_grupo(nomeGrupo, lista_ativos,img):
        # ler grupo do Whatsapp
        lista_junia = read_file(nomeGrupo)
        # define var apenas com a coluna "Phone Number" da planilha
        gp_junia_num = list(lista_junia["Phone Number \t\t\t\t\t"])
        ...
        title = f"Junias que estão no grupo {nomeGrupo[:-5].upper()} e não estão ATIVOS no CADASTRO: \n\n"

        title2, msg2 = checkar_ativos(nomeGrupo, lista_ativos)
        ...
        bot = {
            ...
            'title_msg':f'Compara grupo {nomeGrupo[:-5].upper()} com junias ATIVOS no cadastro',
    ...
    checkar_grupo(nomeGrupo, nome_num_ativo, imgGrupo)
...
#verificaGrupos('Tsubame 2022', 'Cadastro Tsubame', 'Tsubame', '🥁 [WADAIKO] Tsubame 燕.xlsx', 'https://i.imgur.com/7XeLpEa.jpg')
